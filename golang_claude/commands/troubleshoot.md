...
2. **Check Application Health**
   - Verify the CLI tool builds: `go build ./...`
...
9. **Validate the Fix**
   - Run relevant tests based on affected components:
     - All tests: `go test ./...`
     - Specific package: `go test ./pkg/specific/...`
     - With coverage: `go test -coverprofile=coverage.out ./...`
...
- `internal/cli/` - CLI command definitions (cobra commands)
- `internal/config/` - Configuration management
- `internal/` - Other private application code
