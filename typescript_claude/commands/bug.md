...
- `cd backend && npm test` - Run backend tests to validate the bug is fixed with zero regressions
- `cd frontend && npx tsc --noEmit` - Run frontend TypeScript check to validate the bug is fixed with zero regressions
